...
%ccs add
   function res = backwardfromto(self,start_layer_id,end_layer_id)
    %  CHECK(iscell(output_diff), 'output_diff must be a cell array');
    %  CHECK(length(output_diff) == length(self.outputs), ...
    %    'output diff cell length must match output blob number');
      % copy diff to output blobs
    ...
    %  end
      %self.backward_prefilled();
          caffe_('net_backwardfromto', self.hNet_self,start_layer_id,end_layer_id);
      % retrieve diff from input blobs
      res = cell(length(self.inputs), 1);
      for n = 1:length(self.inputs)
        res{n} = self.blobs(self.inputs{n}).get_diff();
      ...
      function res = forwardfromto(self,input_data,start_layer_id,end_layer_id)
    %  CHECK(iscell(output_diff), 'output_diff must be a cell array');
    %  CHECK(length(output_diff) == length(self.outputs), ...
    %    'output diff cell length must match output blob number');
      % copy diff to output blobs
    ...
    %  end
      ...
      caffe_('net_forwardfromto', self.hNet_self,start_layer_id,end_layer_id);
      % retrieve data from output blobs
      res = cell(length(self.outputs), 1);
      for n = 1:length(self.outputs)
        res{n} = self.blobs(self.outputs{n}).get_data();
...
%ccs done
    ...
    function set_relu_mask_control(self,on_or_off)
         caffe_('set_relu_mask_control',self.hNet_self,on_or_off);
    end
    function reset_all_relu_gates(self,value)
         caffe_('reset_all_relu_gates',self.hNet_self,value);
    end

    function set_all_relus_threshold_ratio(self,thd)
         caffe_('set_all_relus_threshold_ratio',self.hNet_self,thd);
    end
    function set_all_relus_bp_mode(self,mode)
         caffe_('set_all_relus_bp_mode',self.hNet_self,mode);
    end
