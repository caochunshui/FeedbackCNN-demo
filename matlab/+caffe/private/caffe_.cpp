...
static void net_forwardfromto(MEX_ARGS) {
  mxCHECK(nrhs == 3 && mxIsStruct(prhs[0]),
      "Usage: caffe_('net_forwardfromto', hNet,start,end)");
  Net<float>* net = handle_to_ptr<Net<float> >(prhs[0]);
  int start_id = mxGetScalar(prhs[1]);
  int end_id = mxGetScalar(prhs[2]);
  net->ForwardFromTo(start_id,end_id);
}
static void set_relu_mask_control(MEX_ARGS){
   Net<float>* net = handle_to_ptr<Net<float> >(prhs[0]);
   net->set_relu_mask_control(mxGetScalar(prhs[1]));
}
static void set_all_relus_bp_mode(MEX_ARGS){
   Net<float>* net = handle_to_ptr<Net<float> >(prhs[0]);
   net->set_all_relus_bp_mode(mxGetScalar(prhs[1]));
}

static void reset_all_relu_gates(MEX_ARGS){
   Net<float>* net = handle_to_ptr<Net<float> >(prhs[0]);
   net->reset_all_relu_gates(mxGetScalar(prhs[1]));
}

static void set_all_relus_threshold_ratio(MEX_ARGS){
   Net<float>* net = handle_to_ptr<Net<float> >(prhs[0]);
   net->set_all_relus_threshold_ratio(mxGetScalar(prhs[1]));
}


// Usage: caffe_('net_backward', hNet)
static void net_backward(MEX_ARGS) {
  mxCHECK(nrhs==1&&mxIsStruct(prhs[0]), "Usage: caffe_('net_backward', hNet)");
  Net<float>* net = handle_to_ptr<Net<float> >(prhs[0]);
  net->Backward();
}
...
static void net_backwardfromto(MEX_ARGS) {
  mxCHECK(nrhs==3&&mxIsStruct(prhs[0]),
      "Usage: caffe_('net_backward', hNet,start,end)");
  Net<float>* net = handle_to_ptr<Net<float> >(prhs[0]);
  int start_id = mxGetScalar(prhs[1]);
  int end_id = mxGetScalar(prhs[2]);

  net->BackwardFromTo(start_id,end_id);
}
...
static void saliency_cut(MEX_ARGS){

cv::Mat img = MxArray(prhs[0]).toMat();
cv::Mat sal = MxArray(prhs[1]).toMat();
cv::Mat output = MxArray(prhs[2]).toMat();
  ...
  saliencyCut(img, sal, output);
  //cv::imwrite("hh.jpg",img); 
  //cv::imwrite("hhh.jpg",sal); 
  //cv::imwrite("hhhh.jpg",output); 
 // for( unsigned int i =2000; i < 1000; ++i)  
...
static handler_registry handlers[] = {
  ...
  { "set_relu_mask_control", set_relu_mask_control},
  { "set_all_relus_threshold_ratio", set_all_relus_threshold_ratio},
  { "set_all_relus_bp_mode", set_all_relus_bp_mode},
  { "reset_all_relu_gates", reset_all_relu_gates},
  { "saliency_cut", saliency_cut},
  // The end.
  { "END",                NULL            },
};
