...
template <typename Dtype>
void Net<Dtype>::set_relu_mask_control(bool on_or_off){

    std::string relu("ReLU");
    for(int layer_id=0;layer_id<layers_.size();++layer_id){
    std::string type(layers_[layer_id]->type());
      if(relu==type){
          LOG(INFO) << "set_relu_mask"; 
         ReLULayer<float>*relu_layer=dynamic_cast<ReLULayer<float>*>(layers_[layer_id].get());
         relu_layer->turn_on_activation(on_or_off);
      }
    }
}

template <typename Dtype>
void Net<Dtype>::set_all_relus_bp_mode(int mode){
    ...
    std::string type(layers_[layer_id]->type());
      if(relu==type){
         ReLULayer<float>*relu_layer=dynamic_cast<ReLULayer<float>*>(layers_[layer_id].get());
         relu_layer->set_bp_mode(mode);
      }
    }
}

template <typename Dtype>
void Net<Dtype>::reset_all_relu_gates(Dtype on_or_off){
    ...
    std::string type(layers_[layer_id]->type());
      if(relu==type){
         ReLULayer<float>*relu_layer=dynamic_cast<ReLULayer<float>*>(layers_[layer_id].get());
         relu_layer->reset_all_gates(on_or_off);
      }
    }
}

template <typename Dtype>
void Net<Dtype>::set_all_relus_threshold_ratio(Dtype threshold_ratio){
    ...
    std::string type(layers_[layer_id]->type());
      if(relu==type){
         ReLULayer<float>*relu_layer=dynamic_cast<ReLULayer<float>*>(layers_[layer_id].get());
         relu_layer->set_threshold_ratio(threshold_ratio);
      }
    }
}
  ...
  if(param.turn_on_relu_bp_mask()){
    ...
    std::string type(layers_[layer_id]->type());
      if(relu==type){
          LOG(INFO) << "set_relu_mask"; 
         ReLULayer<float>*relu_layer=dynamic_cast<ReLULayer<float>*>(layers_[layer_id].get());
         relu_layer->turn_on_activation(true);
      }
    }
  }
